...
public:
	...
	stack<Vertex*> myst;//存储来时的所用顶点


	//根据顶点名字返回其索引
	int index(string v, vector<Vertex*> vertexs) {
		for (Vertex* e : vertexs) {
			if (e->name == v) {
				return e->id;
			}
		}
	...
	//输出全部路径
	void path(vector<vector<Vertex*>> prev, vector<Vertex*> vertexs) {
		for (int i = 0; i < vertexs.size(); i++) {
			for (int j = 0; j < vertexs.size(); j++) {
				cout << "[" << vertexs[i]->name << "," << vertexs[j]->name << "]: " << "[";
				if (i == j) {
					cout << vertexs[i]->name <<"," << vertexs[j]->name << "]" << endl;
					continue;
				}
				int f = j;
				while (i!=f) {
					Vertex* temp = prev[i][f];
					myst.push(temp);
					f = index(temp->name, vertexs);
				}
				while (!myst.empty()) {
					Vertex* tp = myst.top();
					myst.pop();
	...
	void FloydWarshall(Graph* s) {
		...
		path(prev, vertexs);//输出顶点到顶点的全部路径
		cout << "-------------------------------" << endl;
	}
